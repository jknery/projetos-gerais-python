...
class LetrasMus(Scrapper):
    ...
    def getArtistaPorLetra(self, letra = None):
        ...
        url = "/letra/" + letra.upper() + "/artistas.html"
        soap = self.makeRequest(url)
        
        body = soap.find('body')
        tbl = body.find('div', attrs={'class' : 'artistas-a js-alphabet-cnt'})
        arts = tbl.find('ul', attrs={'class' : "cnt-list cnt-list--col3"}).find_all('li')
    ...
    def getMusicasPorArtistas(self, artist = None):
        ...
        body = soap.find('body')
        tbl = body.find('div', attrs={'class':'cnt-list--alp'})
        mscs = tbl.find('ul', attrs={'class':'cnt-list'}).find_all('li')
